...
struct nlist* findsym(string s) {
    struct nlist *np;
    for (np=hashtab[gethash(s)]; np != NULL; np = np->next)
         if (s == (*np).name)
            return (np);
    return(NULL);
}
...
struct nlist* installsym(string name, string defn) {
    ...
    if ((np = findsym(name)) != NULL) //found
        (*np).defn = defn; //supersede existing defn
    else { //not found
        np  = new nlist;
        (*np).name = name;
        (*np).defn = defn;
        int hashval = gethash(name);
...
void printTab() {
    ...
    for (int i=0; i<HASHSIZE; i++) {
        ...
        for (np=hashtab[i]; np != NULL; np = np->next) {
            printf("name=%s, defn=%s -> ", (*np).name.c_str(), (*np).defn.c_str());
        }
        printf("NULL\n");
    }
}
